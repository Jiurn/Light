    ...
    <li className="flex gap-4 border-b px-4 py-4">
        ...
        <div className="flex flex-grow flex-col">
            <div className="flex gap-1">
                <Link href={`/profile/${user.id}`} className="font-bold hover:underline">{user.name}</Link>
...
function HeartButton({isLoading, onClick, likedByMe, likeCount}: HeartButtonProps){
    ...
    return (
    <button disabled={isLoading} onClick={onClick} className={`group -ml-2 items-center gap-1 self-start flex transition-colors duration-200 ${likedByMe ? "text-red-500" : "text-gray-500 hover:text-red-500 focus-visible:text-red-500"}`}>
        <IconHoverEffect red>
